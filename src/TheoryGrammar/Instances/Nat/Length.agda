...
{-
  ...
  `Strings/Base`, `Bags/Base` and `Nat/Base` each used to declare their
  OWN `data MonOp`.  Being definitionally distinct types, they made
  `ModelHom strModel natModel` unstatable, and this file rebuilt the
  ℕ-model by hand over the strings `monoidSig`.  All three now import
  `TheoryGrammar.Theories.Monoid` and `natModel` below is literally
  `Nat/Base`'s `Fibered` with its point.  `Nat/LengthFib.lenIx :
  Reindex St.strFib Nt.natFib` is the check: a `Reindex` needs ONE
  signature, so its existence is the statement that the merge happened.
-}
...
-- same names as the string instance, so only the ℕ `Fibered` and its
...
-- which is the point: the substrate-level `⊗ˢ` needs only `Split`.
...
-- and the two `monoidSig`s were definitionally distinct -- so `ModelHom
-- strModel natModel` could not be stated across them.
...
-- PRIMITIVE (phase 1): THE CUT. Given a decomposition i + j = |w| of the
-- LENGTH, produce the decomposition of `w` itself.
cut : (i j : ℕ) (w : String)
    → i + j Eq.≡ length w
...
-- The `appop` instance: two lifts of the same ℕ-splitting agree slotwise.
...
-- And the same statement about `cut` itself: whatever decomposition of `w`
-- has the prescribed lengths, `cut` already found it.
...
-- CROSSING BETWEEN ⊗ˢ AND ⊗[_]. `Reinterpret` is stated at the MODEL-level
-- ⊗ (the one carrying an equation), while programs are written against the
-- SUBSTRATE-level ⊗ˢ.
...
-- ... which is exactly HONESTY of the string point: every splitting
-- recomposes to the world it split.  With that, the forward bridge is
-- not a primitive at all -- it is the generic `Fibered.Bridge`, at
-- EVERY operation rather than just `appop`.
strHonest : Honest strPoint
strHonest nilop []      sp = Eq.refl
strHonest appop w (u , v , s) = split3app s

module SBridge = Bridge strPoint strHonest

⊗ˢ→⊗ : {A : Bool → Gr} → ⊗ˢ appop A ⊢ ⊗[ appop ] A
⊗ˢ→⊗ {A = A} = SBridge.⊗ˢ→⊗ appop {A = A}
...
-- The SAME translation on the ℕ side. `transportGF` below is stated at
-- `ℕM.⊗[ appop ]`, because that is the connective `ChangeOfTheory` speaks.

-- ... and the same on the ℕ side, from the same generic bridge.
natHonest : Honest N.natPoint
natHonest nilop zero    sp = Eq.refl
natHonest appop n (i , j , a) = N.add3→+ a

module NBridge = Bridge N.natPoint natHonest

⊗ˢᴺ→⊗ᴺ : {A : Bool → NatGr} → N.⊗ˢ appop A ℕM.⊢ ℕM.⊗[ appop ] A
⊗ˢᴺ→⊗ᴺ {A = A} = NBridge.⊗ˢ→⊗ appop {A = A}

-- transport-free by hand, as above.  The `λ { true → … ; false → … }` is
-- the arity-η tax: `if a then m⃗ true else m⃗ false` is not `m⃗ a`.
...
-- (1) The multiplicative half, in both directions.
pull⊗ˢ : (B : Bool → NatGr)
...
-- (3) THE STATEMENT WORTH READING. A generating-function identity A(x) ≅
-- B(x) · C(x) proved once over ℕ becomes an isomorphism of STRING
-- GRAMMARS, with no further work and no reference to strings in its proof.
