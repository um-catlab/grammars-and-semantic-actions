...
{-
  ...
  failures.
-}
module TheoryGrammar.Instances.Spans.Tests where
...
charAt : ℕ → Bool
charAt zero                      = true
charAt (suc zero)                = true
charAt (suc (suc zero))          = true
charAt (suc (suc (suc _)))       = false
...
-- ==================================================================
-- THE CHART, ROW BY ROW.
--
-- Each battery is EVERY span of a given length, so the suite pins the
-- whole row and not just its successes.
-- ==================================================================

-- length 1 -- the base of the chart.  `A` is the first three positions,
-- `B` the last three; `S` and `C` are never letters.
...
_ : passes (run (derives! ntS) at
             ( (0 , 1) ↦ false ∷ (2 , 1) ↦ false ∷ (5 , 1) ↦ false ∷ [] ))
_ = refl

-- length 2 -- "aa" "aa" "ab" "bb" "bb".  Only the middle one is an S.
_ : passes (run (derives! ntS) at
             ( (0 , 2) ↦ false ∷ (1 , 2) ↦ false ∷ (2 , 2) ↦ true
             ∷ (3 , 2) ↦ false ∷ (4 , 2) ↦ false ∷ [] ))
_ = refl

-- length 3 -- "aaa" "aab" "abb" "bbb".  `C` derives aⁿbⁿ⁺¹, so exactly
-- "abb" is a C, and nothing of length 3 is an S (odd length).
_ : passes (run (derives! ntC) at
             ( (0 , 3) ↦ false ∷ (1 , 3) ↦ false
             ∷ (2 , 3) ↦ true  ∷ (3 , 3) ↦ false ∷ [] ))
_ = refl
...
-- length 4 -- "aaab" "aabb" "abbb".  Exactly "aabb" is an S.
_ : passes (run (derives! ntS) at
             ( (0 , 4) ↦ false ∷ (1 , 4) ↦ true ∷ (2 , 4) ↦ false ∷ [] ))
_ = refl

-- length 5 -- "aaabb" "aabbb".  `C` takes the second, `S` neither.
_ : passes (run (derives! ntC) at
             ( (0 , 5) ↦ false ∷ (1 , 5) ↦ true ∷ [] ))
_ = refl
...
-- length 6 -- the whole word, `a³b³`.  The one span the grammar was
-- written for, and the deepest nesting in the suite.
_ : passes (run (derives! ntS) at ( (0 , 6) ↦ true ∷ [] ))
_ = refl

_ : passes (run (derives! ntC) at ( (0 , 6) ↦ false ∷ [] ))
_ = refl

-- ==================================================================
-- ... and the answers are THEOREMS, not reports.  `witness` and
-- `refute` read the same term at the same spans; a parser (error `⊤G`)
-- would give `tt` for the negative one.
-- ==================================================================

a3b3 : Deriv ntS (0 , 6)
a3b3 = witness (Deriv ntS) (¬G Deriv ntS) (derives? ntS) (0 , 6) refl

-- "aaab" is not an S, and this is a proof
no-a3b : Deriv ntS (0 , 4) → E.⊥* {ℓ-zero}
no-a3b = refute (Deriv ntS) (¬G Deriv ntS) (derives? ntS) (0 , 4) refl

-- ... and the inner nonterminal is decided too, positively and negatively
abb-is-C : Deriv ntC (2 , 3)
abb-is-C = witness (Deriv ntC) (¬G Deriv ntC) (derives? ntC) (2 , 3) refl

no-aaa-C : Deriv ntC (0 , 3) → E.⊥* {ℓ-zero}
no-aaa-C = refute (Deriv ntC) (¬G Deriv ntC) (derives? ntC) (0 , 3) refl
