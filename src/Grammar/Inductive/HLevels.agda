...
module _ {X : Type ℓX} where
  ...
  opaque
    ...
    FP : (F : SPFunctor X) → (w : String) → FS F w → Type ℓX
    FP (k g) w s = ⊥*
    FP (Var x') w s = Unit*
    ...
    F-inX : (F : SPFunctor X) (ix : X × String) (s : FS F (ix .snd))
      (p : FP F (ix .snd) s)
      → X × String
  ...
  μIW : (X → SPFunctor X) → X × String → Type ℓX
  μIW F = IW
    (λ ix → FS (F (ix .fst)) (ix .snd))
    (λ ix → FP (F (ix .fst)) (ix .snd))
    λ ix → F-inX (F (ix .fst)) ix

  getShapeF : {A : X → Grammar ℓA}(F : SPFunctor X)
    → ∀ w
    → ⟦ F ⟧ A w → FS F w
  ...
  opaque
    ...
    getSubtreeF : (A : X → Grammar ℓA) (F : SPFunctor X)
      → ∀ w x
      → (e : ⟦ F ⟧ A w)
    ...
    nodeF : ∀ {A : X → Grammar ℓA}(F : SPFunctor X)
      → ∀ w x
      → (s : FS F w)
      → (∀ (p : FP F w s) →
    ...
    reconstructF : ∀ {A : X → Grammar ℓA}(F : SPFunctor X)
      → ∀ w x
      → (t : ⟦ F ⟧ A w)
  ...
  {-# TERMINATING #-}
  encode : (F : X → SPFunctor X) → ∀ ix → μ F (ix .fst) (ix .snd) → μIW F ix
  encode F (x , w) (roll .w op⟨m⟩) =
    node (Inductive.map (F x) (λ _ _ → _) w op⟨m⟩)
      λ p → encode F _ (getSubtreeF (μ F) (F x) w x _ p)

  decode : (F : X → SPFunctor X) → ∀ ix → μIW F ix → μ F (ix .fst) (ix .snd)
