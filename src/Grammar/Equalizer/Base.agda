...
module _ {X : Type ℓX} (F : X → SPFunctor X) (A : X → Grammar ℓA)
  (e e' : ∀ (x : X) → μ F x ⊢ A x)
  (pf : ∀ (x : X) →
    e  x ∘g roll ∘g map (F x) (λ x' → eq-π (e x') (e' x')) ≡
    e' x ∘g roll ∘g map (F x) (λ x' → eq-π (e x') (e' x'))) where

  equalizer-ind-alg : ∀ x → ⟦ F x ⟧ (λ x → equalizer (e x) (e' x)) ⊢ equalizer (e x) (e' x)
  equalizer-ind-alg x =
    eq-intro (e x) (e' x)
...
equalizer-ind-unit : (F : SPFunctor Unit) {A : Grammar ℓA}
  {e e' : μ (λ _ → F) tt ⊢ A}
